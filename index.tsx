...
function renderAppView() {
    if (currentUser) {
        ...
        // Restore header for logged-out user (it will be hidden by CSS)
...
async function handleShareReport(e: Event) {
    ...
    if (!currentUser || !currentUser.performanceReviews || currentUser.performanceReviews.length === 0) {
    ...
    try {
        const latestReview = currentUser.performanceReviews[currentUser.performanceReviews.length - 1];
        const latestReviewJSON = JSON.stringify(latestReview);

        const prompt = `Summarize the following financial performance review into a concise, encouraging email report for a business owner named ${currentUser.name}. The target recipient is ifezued@gmail.com. Highlight the budget adherence score, the biggest area of overspending, and the biggest win. Format it as a simple text email body. Performance Review Data: ${latestReviewJSON}`;
...
function liquidateInvestment(investmentId: string) {
    ...
    if (confirm("Are you sure you want to liquidate this investment? The funds will be returned to your wallet.")) {
        ...
        addFeedItem('action', `Liquidated investment in ${investment.name}.`);
        saveUserDatabase();
        renderWalletOverview();
        renderDashboardSummary();
        renderInvestments();
...
function openVarianceDetailsModal(category: string) {
    ...
    const currency = currentUser.budgets[0]?.summary.currency || '₦';
    const expenseItems = currentUser.expenses
        .flatMap(report => report.categorizedExpenses.filter(e => e.category === category));
    ...
    if (expenseItems.length === 0) {
        ...
        `).join('');
        varianceDetailsList.innerHTML = `<div class="transaction-list-modal">${expenseHtml}</div>`;
        renderIcons();
    }
...
function handleDeleteOpportunities() {
    if (!currentUser || !currentUser.opportunities || currentUser.opportunities.length === 0) return;
    
    if (confirm("Are you sure you want to delete the latest opportunities report?")) {
        currentUser.opportunities.pop();
        saveUserDatabase();
        renderSavedOpportunities();
        showStatusMessage(opportunitiesStatusArea, "Opportunities report deleted.", "success");
        hideStatusMessage(opportunitiesStatusArea, 3000);
    }
}
...
function initialize() {
    ...
    if(!PAYSTACK_PUBLIC_KEY) {
        console.error("PAYSTACK_PUBLIC_KEY is missing. Payment features will be disabled.");
        document.getElementById('config-error-banner')?.classList.remove('hidden');
    } else {
    ...
    footerSignUpBtn?.addEventListener('click', () => openAuthModal(true));
    ...
    document.body.addEventListener('click', (e: Event) => {
        ...
        // Onboarding checklist item clicks
        const onboardingItem = target.closest('.getting-started-item') as HTMLElement;
        if (onboardingItem?.dataset.view) {
            navigateToView(onboardingItem.dataset.view as AppView);
        } else if (onboardingItem?.dataset.action === 'fund-wallet') {
            openFundWalletModal();
        }
        
        // Dashboard overview card clicks
        const overviewCard = target.closest('.overview-card') as HTMLElement;
        if (overviewCard?.dataset.view) {
            navigateToView(overviewCard.dataset.view as AppView);
        }
    });
    ...
    savedAdCopyLibraryContainer.addEventListener('click', (e) => {
         const target = e.target as HTMLElement;
         const deleteBtn = target.closest('.delete-btn[data-creative-index]');
         if (deleteBtn) {
             const index = parseInt(deleteBtn.getAttribute('data-creative-index')!, 10);
             if (currentUser && confirm(`Are you sure you want to delete ad set ${index + 1}?`)) {
                currentUser.creativeCopies.splice(currentUser.creativeCopies.length - 1 - index, 1);
                saveUserDatabase();
                renderSavedAdCopyLibrary();
             }
         } else {
    ...
    budgetHistoryList.addEventListener('click', (e) => {
         const target = e.target as HTMLElement;
         const deleteBtn = target.closest('.delete-btn[data-budget-index]');
         if (deleteBtn) {
             const index = parseInt(deleteBtn.getAttribute('data-budget-index')!, 10);
             if (currentUser && confirm(`Are you sure you want to delete this budget?`)) {
                currentUser.budgets.splice(currentUser.budgets.length - 1 - index, 1); // since it's reversed
                saveUserDatabase();
                renderBudgetHistory();
             }
         }
    ...
    expensePeriodSelector.addEventListener('click', (e: Event) => {
        const target = e.target as HTMLButtonElement;
        if (target.matches('.segmented-control-item')) {
            ...
            target.classList.add('active');
            const isCustom = target.dataset.period === 'custom';
    ...
    reviewResultsContainer.addEventListener('click', (e) => {
        ...
        if (row) {
            ...
            openVarianceDetailsModal(category);
        }
    });
    ...
    // Header actions (for when header is visible)
    document.getElementById('headerLoginBtn')?.addEventListener('click', () => openAuthModal(false));
    document.getElementById('headerSignUpBtn')?.addEventListener('click', () => openAuthModal(true));
