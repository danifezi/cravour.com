...
const footerSignUpBtn = document.getElementById('footerSignUpBtn') as HTMLButtonElement | null;
...
function handleDirectTransfer(e: Event) {
    ...
    addFeedItem('action', `Sent ${amount.toLocaleString()} to ${recipient}.`);
    saveUserDatabase();
    
    showStatusMessage(transferStatusArea, `Successfully transferred ${amount.toLocaleString()} to ${recipient}.`, 'success');
    transferForm.reset();
...
function setTheme(theme: Theme) {
    ...
    const themeIcon = theme === 'light' ? icons.moon : icons.sun;
    const oppositeTheme = theme === 'light' ? 'dark' : 'light';
    const newAriaLabel = `Toggle to ${oppositeTheme} theme`;
    
    if (themeToggleHeaderBtn) {
        themeToggleHeaderBtn.innerHTML = themeIcon;
        themeToggleHeaderBtn.setAttribute('aria-label', newAriaLabel);
    }
    if (themeToggleSidebarBtn) {
        themeToggleSidebarBtn.innerHTML = themeIcon;
        themeToggleSidebarBtn.setAttribute('aria-label', newAriaLabel);
    }
}
...
function handleAddMerchant(e: Event) {
    ...
    if (!name || !category) {
        showStatusMessage(addMerchantStatus, 'Please fill in all fields.', 'error');
        return;
    }

    if (currentUser.merchants.some(m => m.name.toLowerCase() === name.toLowerCase())) {
        showStatusMessage(addMerchantStatus, 'A merchant with this name already exists.', 'error');
        return;
    }
    ...
    setTimeout(() => hideStatusMessage(addMerchantStatus), 3000);
}

function handleContactForm(e: Event) {
    e.preventDefault();
    showStatusMessage(contactFormMessage, 'Thank you! Your message has been sent.', 'success');
    contactForm.reset();
    setTimeout(() => hideStatusMessage(contactFormMessage), 5000);
}

function handleDeleteOpportunities() {
    if (!currentUser) return;
    if (confirm("Are you sure you want to delete the latest opportunities report?")) {
        currentUser.opportunities.pop();
        saveUserDatabase();
        renderSavedOpportunities();
        addFeedItem('insight', 'Opportunities report was deleted.');
    }
}

function handleShareReport() {
    if (!currentUser || currentUser.performanceReviews.length === 0) {
        showStatusMessage(reviewStatusArea, 'Generate a review first before sharing.', 'info');
        return;
    }
    const email = prompt("Enter the email address to send the report to:", currentUser.email);
    if (email) {
        showStatusMessage(reviewStatusArea, `Report sent to ${email}. (This is a simulation).`, 'success');
        setTimeout(() => hideStatusMessage(reviewStatusArea), 3000);
    }
}
...
function handleOpportunitiesActions(e: Event) {
    if (!currentUser) return;
    const target = e.target as HTMLButtonElement;
    if (!target.matches('[data-action-type]')) return;

    const actionType = target.dataset.actionType!;
    const actionTarget = target.dataset.actionTarget!;
    const opportunityIndex = parseInt(target.dataset.opportunityIndex!, 10);
    
    currentInvestmentOpportunity = currentUser.opportunities[currentUser.opportunities.length-1].opportunities[opportunityIndex];
    ...
    if (actionType === 'navigate') {
        ...
        maxInvestmentAmount.textContent = `Available in vault: ${currentUser.budgets[0]?.summary.currency || '₦'}${currentUser.savingsVault.toLocaleString()}`;
        (investmentAmountInput as HTMLInputElement).max = currentUser.savingsVault.toString();
...
function handleLiquidateInvestment(e: Event) {
    const target = e.target as HTMLButtonElement;
    if (!currentUser || !target.matches('[data-investment-id]')) return;
    ...
    if (confirm(`Are you sure you want to liquidate your investment in "${investment.name}"? This is a simulation.`)) {
        const liquidatedAmount = investment.amount;
        currentUser.wallet.balance += liquidatedAmount;
        currentUser.wallet.transactions.push({
            id: `liq_${Date.now()}`,
            date: new Date().toISOString(),
            type: 'liquidation',
        ...
        addFeedItem('achievement', `Successfully liquidated ${investment.name} for ${liquidatedAmount.toLocaleString()}.`);
        saveUserDatabase();
        renderInvestments();
        renderWalletOverview();
    }
...
function handleInvestmentSimulation(e: Event) {
    ...
    addFeedItem('achievement', `You invested ${amount.toLocaleString()} in "${currentInvestmentOpportunity.title}"!`);
    saveUserDatabase();
...
function handleMerchantHistory(e: Event) {
    if (!currentUser) return;
    const target = e.target as HTMLElement;
    const merchantItem = target.closest<HTMLElement>('.merchant-item');
    if (!merchantItem || target.closest('.delete-btn')) return;

    const merchantName = merchantItem.dataset.merchantName!;
    merchantHistoryName.textContent = merchantName;
    
    const relevantExpenses = currentUser.expenses
        .flatMap(report => report.categorizedExpenses)
        .filter(expense => (expense.merchantBrandExample || expense.merchantCategory) === merchantName);

    if (relevantExpenses.length === 0) {
        merchantHistoryList.innerHTML = `<div class="empty-state">No transaction history found for this merchant in your expense reports.</div>`;
    } else {
        const currency = currentUser.budgets[0]?.summary.currency || '₦';
        const historyHtml = relevantExpenses.map(expense => `
            <div class="history-item">
                <p><strong>${expense.category}</strong></p>
                <p class="amount error">${currency}${expense.amount.toLocaleString()}</p>
            </div>
        `).join('');
...
function handleVarianceDetails(e: Event) {
    if (!currentUser || currentUser.performanceReviews.length === 0) return;
    const target = e.target as HTMLElement;
    const row = target.closest<HTMLElement>('.variance-row');
    if (!row) return;

    const category = row.dataset.category!;
    varianceCategoryName.textContent = category;
    
    const relevantExpenses = currentUser.expenses[currentUser.expenses.length - 1]
        .categorizedExpenses
        .filter(exp => exp.category === category);
    
    if (relevantExpenses.length === 0) {
        varianceDetailsList.innerHTML = `<div class="empty-state">No specific expenses found for this category in the latest report.</div>`;
    } else {
        const currency = currentUser.performanceReviews[currentUser.performanceReviews.length - 1].currency || '₦';
        const detailsHtml = relevantExpenses.map(exp => `
             <div class="history-item">
                <p><strong>${exp.merchantBrandExample || exp.merchantCategory}</strong></p>
...
function handleDynamicDeletes(e: Event) {
    if (!currentUser) return;
    const target = e.target as HTMLElement;
    ...
    const budgetIndexStr = deleteBtn.dataset.budgetIndex;
    if (budgetIndexStr) {
        if (confirm('Are you sure you want to delete this budget? This may affect your expense analysis and performance reviews.')) {
            const index = parseInt(budgetIndexStr, 10);
            currentUser.budgets.splice(index, 1);
            // Optionally remove corresponding expense/review reports
            if (currentUser.expenses[index]) currentUser.expenses.splice(index, 1);
            if (currentUser.performanceReviews[index]) currentUser.performanceReviews.splice(index, 1);
            saveUserDatabase();
            renderBudgetHistory();
        }
        return;
    ...
    const creativeIndex = deleteBtn.dataset.creativeIndex;
    if (creativeIndex) {
        if (confirm('Are you sure you want to delete this ad copy set?')) {
    ...
    const merchantName = deleteBtn.dataset.merchantDeleteName;
    if (merchantName) {
...
function initApp() {
    ...
    // Event Listeners
    landingHamburger.addEventListener('click', handleLandingNavToggle);
    sidebarHamburger.addEventListener('click', handleSidebarToggle);
    appOverlay.addEventListener('click', handleSidebarToggle);
    
    themeToggleHeaderBtn.addEventListener('click', handleThemeToggle);
    themeToggleSidebarBtn.addEventListener('click', handleThemeToggle);

    [ctaSignUpBtn, footerSignUpBtn, document.getElementById('headerSignUpBtn')].forEach(btn => btn?.addEventListener('click', () => openAuthModal(true)));
    ...
    sidebarMenu.addEventListener('click', (e) => {
        ...
        if (link?.dataset.view) {
            e.preventDefault();
            navigateToView(link.dataset.view as AppView);
        }
    });
    
    gettingStartedChecklist.addEventListener('click', (e) => {
        const item = (e.target as HTMLElement).closest<HTMLElement>('.getting-started-item');
        if (!item) return;
        if (item.dataset.view) navigateToView(item.dataset.view as AppView);
        else if (item.dataset.action === 'fund-wallet') openFundWalletModal();
    });
    ...
    budgetHistoryList.addEventListener('click', handleDynamicDeletes);
    ...
    expensePeriodSelector.addEventListener('click', (e) => {
        ...
        if (target.matches('.segmented-control-item')) {
            expensePeriodSelector.querySelectorAll('.segmented-control-item').forEach(item => item.classList.remove('active'));
            target.classList.add('active');
    ...
    reviewResultsContainer.addEventListener('click', handleVarianceDetails);
    
    creativeSuiteContainer.querySelector('.creative-suite-tabs')?.addEventListener('click', handleCreativeSuiteTabs);
    creativeSuiteFormDashboard.addEventListener('submit', handleGenerateCreativeCopyDashboard);
    creativeSuiteContainer.addEventListener('click', handleCopyActions);
    savedAdCopyLibraryContainer.addEventListener('click', handleDynamicDeletes);
    ...
    opportunitiesResultsContainer.addEventListener('click', handleOpportunitiesActions);
    ...
    allMerchantsListWallet.addEventListener('click', (e) => {
        handleDynamicDeletes(e);
        handleMerchantHistory(e);
    });
    paymentList.addEventListener('click', handlePaymentListActions);
    investmentList.addEventListener('click', handleLiquidateInvestment);
    ...
    investmentSimulatorForm.addEventListener('submit', handleInvestmentSimulation);
}
