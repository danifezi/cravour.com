...
function renderReviewView() {
    // This view is now simpler, just the review generation part
}
    ...
    const merchantsHtml = `
        <div class="result-section">
            ...
            <div class="merchant-grid">
                ${plan.recommendedMerchants.map((merchant) => `<div class="merchant-card"><h4>${merchant.name} ${merchant.verified ? `<span class="verified-icon">${icons.check}</span>` : ''}</h4><p><span class="footer-icon-placeholder" data-icon="envelope"></span> ${merchant.address}</p><p class="deals">${merchant.deals}</p></div>`).join('')}
            </div>
        </div>`;
...
function renderIcons() {
    document.querySelectorAll<HTMLElement>('[data-icon]').forEach(el => {
        ...
        if (kebabKey) {
            // Convert kebab-case to camelCase (e.g., "info-circle" to "infoCircle")
...
function renderWalletTransactions() {
    ...
    const transactionHtml = transactions.map(t => {
        const isDebit = t.type === 'payment' || t.type === 'transfer';
        const icon = isDebit ? icons.arrowUp : icons.arrowDown;
...
function handleFundWallet(e: Event) {
    ...
    currentUser.wallet.transactions.push({
        ...
        type: 'fund',
        description: 'Wallet funded (Simulated)',
        amount: amount
    });
    
    saveUserDatabase();
    showStatusMessage(fundWalletStatus, `Successfully added ${amount.toLocaleString()} to your wallet.`, 'success');
...
function handlePayWithWallet() {
    ...
    saveUserDatabase();
    showStatusMessage(paymentStatus, `Paid ${currentPayment.amount.toLocaleString()} to ${currentPayment.merchant} from your wallet.`, 'success');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (configErrorBanner && (!isAiConfigured)) {
