...
async function handleLogin(e: Event) {
    ...
    showStatusMessage(statusEl, "Logging in...", 'loading');
...
async function handleRegister(e: Event) {
    ...
    showStatusMessage(statusEl, "Creating account...", 'loading');
    button.disabled = true;
    try {
...
async function handleGeneratePlanLanding(e: Event) {
    ...
    if (description.trim().length < 10) {
        return showStatusMessage(statusContainer, "Please provide a more detailed shopping goal.", 'error');
    }

    showStatusMessage(statusContainer, "Generating your intelligent plan...", 'loading');
...
async function fetchAndRenderPlans() {
    ...
    showStatusMessage(plansGrid, 'Loading your plans...', 'loading');
    try {
...
async function handleAddTransaction(e: Event) {
    ...
    try {
        ...
        showToast(error.response?.data?.error || 'Failed to add transaction.', 'error');
    } finally {
        button.disabled = false;
    }
}
...
function handleFundWallet(e: Event) {
    ...
    if (isNaN(amount) || amount < 100) {
        return showToast('Please enter a valid amount (minimum ₦100).', 'error');
    }
...
async function handleGenerateAd(e: Event) {
    ...
    showToast('Generating brilliant ad creative...', 'info');
    button.disabled = true;
...
function renderAdCopy(copy: AdCopy, container: HTMLElement) {
    ...
    imageEl.src = copy.imageUrl;
    imageEl.onload = () => {
        const placeholder = container.querySelector('.image-placeholder');
        if (placeholder) {
            placeholder.replaceWith(imageEl);
        }
    };
