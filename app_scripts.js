...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (tomorrowBtn && planDateInput) {
        tomorrowBtn.addEventListener('click', function() {
            ...
            (planDateInput as HTMLInputElement).value = tomorrow.toISOString().split('T')[0];
        });
    }
...
async function loadDashboardData() {
    ...
    try {
        ...
        if (!responseText) {
            throw new Error("AI returned an empty response for dashboard data.");
        }
...
function renderDashboard(data) {
    ...
    if (barChartContainerEl && data.spendingByCategory) {
        ...
        barChartContainerEl.innerHTML = data.spendingByCategory.map((cat) => {
    ...
    if (transactionTableBodyEl && data.transactions) {
        transactionTableBodyEl.innerHTML = data.transactions.map((tx) => {
